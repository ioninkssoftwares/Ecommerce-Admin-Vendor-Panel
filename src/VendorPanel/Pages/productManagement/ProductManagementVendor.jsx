  ...
  const getProductsByVendors = async () => {
    ...
    try {
      const res = await instance.get(
        `/product/getProductByVendorId/${vendorId}`
      );
      if (res.data) {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      if (vendorProducts.length > 0) {
        const vendorsData = vendorProducts.map(
          ({ name, brand, category, subCategory, price, stock }) => ({
            Name: name,
  ...
  const all_customer_columns = [
...

    {
      ...
      renderCell: ({ row }) => {
        ...
        const isActive = row.isVerified === "true" || row.isVerified === true;
      ...
      renderCell: ({ row }) => (
        <Box>
          <Tooltip title="View Order Details">
            <IconButton
              onClick={() => navigate(`/vendor/product/${row._id}`)}
              color="error"
            >
              <FaEye />
            </IconButton>
